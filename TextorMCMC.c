...
double getLih_1()
{
	...
	for(m=1.36;m<1.8;m=m+dm)
	{
		...
		dLih31=dLih11/3;
		dLih61=dLih11/6;
		dLih22=PDF_m1(m+dm/2)*PDF_Lambda1(interp_L(m+dm/2))*dm/2;
		dLih62=PDF_m1(m+dm/6)*PDF_Lambda1(interp_L(m+dm/6))*dm/6;
...
double getLih_2()
{
	...
	for(m=1.07;m<1.4;m=m+dm)
	{
		...
		dLih31=dLih11/3;
		dLih61=dLih11/6;
		dLih22=PDF_m2(m+dm/2)*PDF_Lambda2(interp_L(m+dm/2))*dm/2;
		dLih62=PDF_m2(m+dm/6)*PDF_Lambda2(interp_L(m+dm/6))*dm/6;
		Lih=Lih+dLih22+9*(dLih62-dLih31);
	}
	return Lih;
}
...
int main()
{
	...
	printf("Welcome to use TextorMCMC --- version 1.18\n========================================\n\n");
