...
int main()
{
	...
	printf("Generating Posterior von Resamples --- version 1.0\n========================================\n\nDistributed Computing Part 5\n\n");
	if((Pin=fopen("ReSamPara_p5","r"))==NULL)
	{
		printf("missing file ReSamPara at root path!\nCaution: this *.exe file cannot run independently.");
		return -1;
	}
	Pout=fopen("Posterior_Results_p5.txt","w");
	Dataout=fopen("Posterior_InternalData_p5.txt","w");
	strucpfout=fopen("Posterior_DataStrucPf_p5","w");
