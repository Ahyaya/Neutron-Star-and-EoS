...
int ScanMode()
{
	double Ec=1e18,dE=5e16,lastM,lastR,lastLambda;
	struct Results Data;
	int n=0,pf=0;
	char filename[30];
	...
	if(Data.Mmax>2.01)
	{
		...
		for(pf=0;pf<n;pf++)
		{
			fprintf(Xout,"%e\t%lf\t%lf\t%lf\t%lf\n",Data.Ec[pf],Data.M[pf],Data.R[pf],Data.I[pf],Data.Lambda[pf]);
		}
		fclose(Xout);
	}
	return 1;
...
int main()
{
	...
	printf("Welcome to use Textor --- version 1.18\n========================================\n\n");
	system("mkdir Results");
	
	Pout=fopen("EoSpara.txt","w");
